...
Begin VB.Form FrmPrintOrderStatusRegister 
   ...
   EndProperty
   Icon            =   "PrinterOrderStatusRegister.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "PrinterOrderStatusRegister.frx":0442
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "PrinterOrderStatusRegister.frx":0986
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "PrinterOrderStatusRegister.frx":0A9A
            Key             =   ""
         EndProperty
      EndProperty
   End
   Begin Mh3dfrmLibCtl.Mh3dFrame Mh3dFrame1 
      ...
      FormatString    =   ""
      Caption         =   ""
      Picture         =   "PrinterOrderStatusRegister.frx":0BAE
      ...
      Begin Mh3dlblLib.Mh3dLabel Mh3dLabel1 
         ...
         Picture         =   "PrinterOrderStatusRegister.frx":0BCA
         Picture         =   "PrinterOrderStatusRegister.frx":0BE6
      End
      Begin Mh3dlblLib.Mh3dLabel Mh3dLabel2 
         ...
         FillColor       =   8421376
         TextColor       =   -2147483634
         Picture         =   "PrinterOrderStatusRegister.frx":0C02
         Picture         =   "PrinterOrderStatusRegister.frx":0C1E
      End
      ...
      Begin TDBDate6Ctl.TDBDate MhDateInput2 
         ...
         _ExtentX        =   1931
         _ExtentY        =   582
         Calendar        =   "PrinterOrderStatusRegister.frx":0C3A
         Caption         =   "PrinterOrderStatusRegister.frx":0D52
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Calibri"
            Size            =   9.75
            Charset         =   0
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "PrinterOrderStatusRegister.frx":0DBE
         Keys            =   "PrinterOrderStatusRegister.frx":0DDC
         Spin            =   "PrinterOrderStatusRegister.frx":0E3A
         AlignHorizontal =   0
         AlignVertical   =   0
         Appearance      =   0
         BackColor       =   16777215
      ...
      Begin TDBDate6Ctl.TDBDate MhDateInput1 
         ...
         _ExtentX        =   1931
         _ExtentY        =   582
         Calendar        =   "PrinterOrderStatusRegister.frx":0E62
         Caption         =   "PrinterOrderStatusRegister.frx":0F7A
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Calibri"
            Size            =   9.75
            Charset         =   0
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "PrinterOrderStatusRegister.frx":0FE6
         Keys            =   "PrinterOrderStatusRegister.frx":1004
         Spin            =   "PrinterOrderStatusRegister.frx":1062
         AlignHorizontal =   0
         AlignVertical   =   0
         Appearance      =   0
         BackColor       =   16777215
...
Private Sub PrintOrderStatusRegister()
    ...
    If InStr(1, "0102", OrderType) > 0 Then 'Book/Print Orderwise
        ...
        SQL = SQL + ",(SELECT Name  FROM PrintPVParent Where Code in( Select Ref From BookPOChild05 WHERE Code=P.Code) ) As Ref1,(SELECT Name  FROM PrintPVParent Where Code in( Select Ref From BookPOChild06 WHERE Code=P.Code) ) As Ref2 ," & _
                             "(SELECT  PDFSendToProduction FROM BookPOChild05 WHERE Code=P.Code) As PDFSendToProduction,(SELECT  PDFSendToPrinter FROM BookPOChild05 WHERE Code=P.Code) As PDFSendToPrinter FROM (((BookPOParent P LEFT JOIN BookPOChild08 C ON P.Code=C.Code) LEFT JOIN AccountMaster M1 ON P.Binder=M1.Code) LEFT JOIN BookMaster M2 ON P.Book=M2.Code) LEFT JOIN GeneralMaster M3 ON M2.Board=M3.Code " & _
    ...
    ElseIf OrderType = "05" Then    'Text Printerwise
        ...
        SQL = SQL + ",(SELECT Name  FROM PrintPVParent Where Code in( Select Ref From BookPOChild05 WHERE Code=P.Code) ) As Ref,C.PDFSendToProduction,C.PDFSendToPrinter ," & _
                             "(SELECT DeliveryDate FROM PaperPOParent WHERE Code IN(Select Code From PaperPOChildRef Where Book+Paper+Ref=P.Book+C.Paper1+C.Ref)) As DeliveryDate1," & _
                             "(SELECT DeliveryDate FROM PaperPOParent WHERE Code IN(Select Code From PaperPOChildRef Where Book+Paper+Ref=P.Book+C.Paper2+C.Ref)) As DeliveryDate2," & _
                             "(SELECT DeliveryDate FROM PaperPOParent WHERE Code IN(Select Code From PaperPOChildRef Where Book+Paper+Ref=P.Book+C.Paper4+C.Ref)) As DeliveryDate3 " & _
                             " FROM (((BookPOParent P INNER JOIN BookPOChild05 C ON P.Code=C.Code) INNER JOIN AccountMaster M1 ON P.BookPrinter=M1.Code) INNER JOIN BookMaster M2 ON P.Book=M2.Code) INNER JOIN GeneralMaster M3 ON M2.Board=M3.Code " & _
                             "WHERE P.Type IN ('" & IIf(Check2.Value And Check3.Value, "F','R", IIf(Check2.Value, "F", "R")) & "') AND C.OrderDate>=#" & GetDate(MhDateInput1.Text) & "# AND C.OrderDate<=#" & GetDate(MhDateInput2.Text) & "# AND " & IIf(Check1.Value, "1=1", "C.Status NOT IN ('D')") & " AND " & IIf(SelectedBoards = "''", "1=1", "M3.Code IN (" & SelectedBoards & ")") & " AND " & IIf(SelectedBooks = "''", "1=1", "M2.Code IN (" & SelectedBooks & ")") & " AND " & IIf(SelectedPrinters = "''", "1=1", "M1.Code IN (" & SelectedPrinters & ")") & Space(1) & _
                             "ORDER BY M1.PrintName,M2.PrintName,Val(P.Name)"
        rstPrintOrderStatusRegister.Open SQL, CxnDatabase, adOpenKeyset, adLockReadOnly
    
    ElseIf OrderType = "06" Then    'Title Printerwise
        SelectedPrinters = SelectedItems(ListView3)
        SQL = SQL + ",(SELECT Name  FROM PrintPVParent Where Code in( Select Ref From BookPOChild06 WHERE Code=P.Code) ) As Ref,C.PDFSendToProduction,C.PDFSendToPrinter," & _
                             "(SELECT DeliveryDate FROM PaperPOParent WHERE Code IN(Select Code From PaperPOChildRef Where Book+Paper+Ref=P.Book+C.Paper+C.Ref)) As DeliveryDate1" & _
                             " FROM (((BookPOParent P INNER JOIN BookPOChild06 C ON P.Code=C.Code) INNER JOIN AccountMaster M1 ON P.TitlePrinter=M1.Code) INNER JOIN BookMaster M2 ON P.Book=M2.Code) INNER JOIN GeneralMaster M3 ON M2.Board=M3.Code " & _
                             " WHERE P.Type IN ('" & IIf(Check2.Value And Check3.Value, "F','R", IIf(Check2.Value, "F", "R")) & "') AND C.OrderDate>=#" & GetDate(MhDateInput1.Text) & "# AND C.OrderDate<=#" & GetDate(MhDateInput2.Text) & "# AND " & IIf(Check1.Value, "1=1", "C.Status NOT IN ('D')") & " AND " & IIf(SelectedBoards = "''", "1=1", "M3.Code IN (" & SelectedBoards & ")") & " AND " & IIf(SelectedBooks = "''", "1=1", "M2.Code IN (" & SelectedBooks & ")") & " AND " & IIf(SelectedPrinters = "''", "1=1", "M1.Code IN (" & SelectedPrinters & ")") & Space(1) & _
                             " ORDER BY M1.PrintName,M2.PrintName,Val(P.Name)"
        rstPrintOrderStatusRegister.Open SQL, CxnDatabase, adOpenKeyset, adLockReadOnly
    ...
    ElseIf InStr(1, "08YY", OrderType) > 0 Then 'Book Binderwise/Pending Debit Note
        ...
        SQL = SQL + ",(SELECT  PDFSendToProduction FROM BookPOChild05 WHERE Code=P.Code) As PDFSendToProduction,(SELECT  PDFSendToPrinter FROM BookPOChild05 WHERE Code=P.Code) As PDFSendToPrinter FROM (((BookPOParent P INNER JOIN BookPOChild08 C ON P.Code=C.Code) INNER JOIN AccountMaster M1 ON P.Binder=M1.Code) INNER JOIN BookMaster M2 ON P.Book=M2.Code) INNER JOIN GeneralMaster M3 ON M2.Board=M3.Code " & _
                             "WHERE P.Type IN ('" & IIf(Check2.Value And Check3.Value, "F','R", IIf(Check2.Value, "F", "R")) & "') AND C.OrderDate>=#" & GetDate(MhDateInput1.Text) & "# AND C.OrderDate<=#" & GetDate(MhDateInput2.Text) & "# AND " & IIf(Check1.Value, "1=1", "C.Status NOT IN ('D','E','W')") & " AND " & IIf(SelectedBoards = "''", "1=1", "M3.Code IN (" & SelectedBoards & ")") & " AND " & IIf(SelectedBooks = "''", "1=1", "M2.Code IN (" & SelectedBooks & ")") & " AND " & IIf(SelectedBinders = "''", "1=1", "M1.Code IN (" & SelectedBinders & ")") & Space(1) & _
                             "ORDER BY M1.PrintName,M2.PrintName,Val(P.Name)"
        rstPrintOrderStatusRegister.Open SQL, CxnDatabase, adOpenKeyset, adLockReadOnly
    
    ElseIf OrderType = "XX" Then    'Pending Print Order (Busy)
        rstPrintOrderStatusRegister.Open "SELECT M2.PrintName As BookName,P.Name As OrderNo,C.OrderDate As OrderDate,C.ActualQuantity As OrderedQuantity,(SELECT PrintName FROM AccountMaster WHERE Code=P.BookPrinter) As BookPrinterName,M1.PrintName As BinderName,(SELECT Alias FROM AccountMaster WHERE Code=P.Laminator) As LaminatorAlias,C.BookEdition, " & _
                                                               "(SELECT  PDFSendToProduction FROM BookPOChild05 WHERE Code=P.Code) As PDFSendToProduction,(SELECT  PDFSendToPrinter FROM BookPOChild05 WHERE Code=P.Code) As PDFSendToPrinter,M2.SaleDiscount,M2.HSNCode,(SELECT  Processing FROM BookPOChild05 WHERE Code=P.Code) As Processing ,M2.Pages,M2.ISBN,M2.Price As MRP FROM (((BookPOParent P INNER JOIN BookPOChild08 C ON P.Code=C.Code) INNER JOIN AccountMaster M1 ON P.Binder=M1.Code) INNER JOIN BookMaster M2 ON P.Book=M2.Code) INNER JOIN GeneralMaster M3 ON M2.Board=M3.Code " & _
                                                               " WHERE P.Type IN ('" & IIf(Check2.Value And Check3.Value, "F','R", IIf(Check2.Value, "F", "R")) & "') AND P.Date>=#" & GetDate(MhDateInput1.Text) & "# AND P.Date<=#" & GetDate(MhDateInput2.Text) & "# AND P.ReceivedQuantity=0 AND " & IIf(SelectedBoards = "''", "1=1", "M3.Code IN (" & SelectedBoards & ")") & " AND " & IIf(SelectedBooks = "''", "1=1", "M2.Code IN (" & SelectedBooks & ")") & Space(1) & _
                                                               " ORDER BY Val(P.Name),M2.PrintName", CxnDatabase, adOpenKeyset, adLockReadOnly
    
    

    Dim aaa As String
    aaa = "SELECT M2.PrintName As BookName,P.Name As OrderNo,C.OrderDate As OrderDate,C.ActualQuantity As OrderedQuantity,(SELECT PrintName FROM AccountMaster WHERE Code=P.BookPrinter) As BookPrinterName,M1.PrintName As BinderName,(SELECT Alias FROM AccountMaster WHERE Code=P.Laminator) As LaminatorAlias,C.BookEdition, " & _
                                                               "(SELECT  PDFSendToProduction FROM BookPOChild05 WHERE Code=P.Code) As PDFSendToProduction,(SELECT  PDFSendToPrinter FROM BookPOChild05 WHERE Code=P.Code) As PDFSendToPrinter,M2.SaleDiscount,M2.HSNCode,(SELECT  Processing FROM BookPOChild05 WHERE Code=P.Code) As Processing ,M2.Pages,M2.ISBN,M2.Price As MRP FROM (((BookPOParent P INNER JOIN BookPOChild08 C ON P.Code=C.Code) INNER JOIN AccountMaster M1 ON P.Binder=M1.Code) INNER JOIN BookMaster M2 ON P.Book=M2.Code) INNER JOIN GeneralMaster M3 ON M2.Board=M3.Code " & _
                                                               " WHERE P.Type IN ('" & IIf(Check2.Value And Check3.Value, "F','R", IIf(Check2.Value, "F", "R")) & "') AND P.Date>=#" & GetDate(MhDateInput1.Text) & "# AND P.Date<=#" & GetDate(MhDateInput2.Text) & "# AND P.ReceivedQuantity=0 AND " & IIf(SelectedBoards = "''", "1=1", "M3.Code IN (" & SelectedBoards & ")") & " AND " & IIf(SelectedBooks = "''", "1=1", "M2.Code IN (" & SelectedBooks & ")") & Space(1) & _
                                                               " ORDER BY Val(P.Name),M2.PrintName"
    ...
    If OrderType <> "XX" Then
        ...
        oExcel.Cells(2, "A").Value = IIf(OrderType = "YY", "Pending Debit Note Register", "Print Order Status Register (" & Choose(Val(OrderType), "Bookwise", "Print Orderwise", "", "", "Book Printerwise", "Title Printerwise", "", "Book Binderwise") & ")") & " From " & Format(MhDateInput1, "dd-MMM-yyyy") & " To " & Format(MhDateInput2, "dd-MMM-yyyy")
        i = 5: Cnt = 1
        Do While Not rstPrintOrderStatusRegister.EOF
            ...
            oExcel.Application.Cells(i, "AE").Value = Format(rstPrintOrderStatusRegister.Fields("TargetDate").Value, "dd-MM-yy") 'Added By Shamahsd
            oExcel.Application.Cells(i, "AF").Value = Format(rstPrintOrderStatusRegister.Fields("ExtendDate").Value, "dd-MM-yy") 'Added By Shamahsd
            
             If InStr(1, "0102", OrderType) > 0 Then
               If Not IsNull(rstPrintOrderStatusRegister.Fields("Ref1").Value) And Not IsNull(rstPrintOrderStatusRegister.Fields("Ref2").Value) Then
                    oExcel.Application.Cells(i, "AG").Value = rstPrintOrderStatusRegister.Fields("Ref1").Value
               ElseIf Not IsNull(rstPrintOrderStatusRegister.Fields("Ref1").Value) And IsNull(rstPrintOrderStatusRegister.Fields("Ref2").Value) Then
                   oExcel.Application.Cells(i, "AG").Value = rstPrintOrderStatusRegister.Fields("Ref1").Value
               ElseIf Not IsNull(rstPrintOrderStatusRegister.Fields("Ref2").Value) And IsNull(rstPrintOrderStatusRegister.Fields("Ref1").Value) Then
                   oExcel.Application.Cells(i, "AG").Value = rstPrintOrderStatusRegister.Fields("Ref2").Value
               End If
            ElseIf OrderType = "05" Then
                 
                 oExcel.Application.Cells(i, "AG").Value = rstPrintOrderStatusRegister.Fields("Ref").Value
                 
                 oExcel.Application.Cells(i, "AH").Value = Format(rstPrintOrderStatusRegister.Fields("PDFSendToProduction").Value, "dd-MM-yy")
                 oExcel.Application.Cells(i, "AI").Value = Format(rstPrintOrderStatusRegister.Fields("PDFSendToPrinter").Value, "dd-MM-yy")
                 
                 If Trim(rstPrintOrderStatusRegister.Fields("DeliveryDate1").Value) <> "" Then
                    oExcel.Application.Cells(i, "AJ").Value = Format(rstPrintOrderStatusRegister.Fields("DeliveryDate1").Value, "dd-MM-yy")
                 End If
                 If Trim(rstPrintOrderStatusRegister.Fields("DeliveryDate2").Value) <> "" Then
                    oExcel.Application.Cells(i, "AJ").Value = Format(rstPrintOrderStatusRegister.Fields("DeliveryDate2").Value, "dd-MM-yy")
                 End If
                 If Trim(rstPrintOrderStatusRegister.Fields("DeliveryDate3").Value) <> "" Then
                    oExcel.Application.Cells(i, "AJ").Value = Format(rstPrintOrderStatusRegister.Fields("DeliveryDate3").Value, "dd-MM-yy")
                 End If
                 
            ElseIf OrderType = "06" Then
                   oExcel.Application.Cells(i, "AG").Value = rstPrintOrderStatusRegister.Fields("Ref").Value
                   oExcel.Application.Cells(i, "AH").Value = Format(rstPrintOrderStatusRegister.Fields("PDFSendToProduction").Value, "dd-MM-yy")
                   oExcel.Application.Cells(i, "AI").Value = Format(rstPrintOrderStatusRegister.Fields("PDFSendToPrinter").Value, "dd-MM-yy")
                   oExcel.Application.Cells(i, "AJ").Value = Format(rstPrintOrderStatusRegister.Fields("DeliveryDate1").Value, "dd-MM-yy")
            ...
            If BreakupExist(Trim(Replace(rstPrintOrderStatusRegister.Fields("OrderNo").Value, "*", "")), rstPrintOrderStatusRegister.Fields(IIf(OrderType = "06", "TitleQuantity", IIf(OrderType = "05", "TextQuantity", "BookQuantity"))).Value) = True Then
               balQty = 0
               balQty = Val(rstPrintOrderStatusRegister.Fields(IIf(OrderType = "06", "TitleQuantity", IIf(OrderType = "05", "TextQuantity", "BookQuantity"))).Value - rstPrintOrderStatusRegister.Fields("ReceivedQuantity").Value)
               oExcel.Application.Cells(i, "L").Value = GetBreakupRecievedQty(Trim(Replace(rstPrintOrderStatusRegister.Fields("OrderNo").Value, "*", "")), RefCode)
               oExcel.Application.Cells(i, "M").Value = balQty
            Else
               oExcel.Application.Cells(i, "L").Value = rstPrintOrderStatusRegister.Fields("ReceivedQuantity").Value
            ...
            For K = 39 To 53
                If IIf(IsNull(rstPrintOrderStatusRegister.Fields("TitleStatus").Value), "N", rstPrintOrderStatusRegister.Fields("TitleStatus").Value) = Trim(oExcel.Application.Cells(K, "XFC")) Then oExcel.Application.Cells(i, "Q").Value = Trim(oExcel.Application.Cells(K, "XFD")): Exit For
            Next
...
'            oExcel.Application.Cells(i, "AE").Value = Format(rstPrintOrderStatusRegister.Fields("TargetDate").Value, "dd-MM-yy")
...
Continue:
    ...
    Else
        ...
        Do While Not rstPrintOrderStatusRegister.EOF
            ...
            oExcel.Cells(i, "A").Value = Cnt
            oExcel.Application.Cells(i, "B").Value = Trim(rstPrintOrderStatusRegister.Fields("BookName").Value)
            oExcel.Application.Cells(i, "C").Value = Trim(rstPrintOrderStatusRegister.Fields("Pages").Value)
            oExcel.Application.Cells(i, "D").Value = Trim(rstPrintOrderStatusRegister.Fields("HSNCode").Value)
            oExcel.Application.Cells(i, "E").Value = Trim(rstPrintOrderStatusRegister.Fields("ISBN").Value)
            oExcel.Application.Cells(i, "F").Value = Trim(rstPrintOrderStatusRegister.Fields("OrderNo").Value)
            oExcel.Application.Cells(i, "G").Value = Format(rstPrintOrderStatusRegister.Fields("OrderDate").Value, "dd-MMM-yyyy")
            oExcel.Application.Cells(i, "H").Value = Format(rstBusy.Fields("Date").Value, "dd-MMM-yyyy")
            oExcel.Application.Cells(i, "I").Value = Val(rstPrintOrderStatusRegister.Fields("OrderedQuantity").Value)
            oExcel.Application.Cells(i, "J").Value = Val(rstBusy.Fields("OrderedQuantity").Value)
            oExcel.Application.Cells(i, "K").Value = Trim(CheckNull(rstPrintOrderStatusRegister.Fields("BookPrinterName").Value))
            oExcel.Application.Cells(i, "L").Value = Trim(CheckNull(rstPrintOrderStatusRegister.Fields("BinderName").Value))
            oExcel.Application.Cells(i, "M").Value = Trim(rstBusy.Fields("BinderName").Value)
            oExcel.Application.Cells(i, "N").Value = Trim(rstPrintOrderStatusRegister.Fields("LaminatorAlias").Value)
            oExcel.Application.Cells(i, "O").Value = Trim(rstPrintOrderStatusRegister.Fields("BookEdition").Value)
            oExcel.Application.Cells(i, "P").Value = Trim(rstPrintOrderStatusRegister.Fields("SaleDiscount").Value)
            oExcel.Application.Cells(i, "Q").Value = Trim(rstPrintOrderStatusRegister.Fields("MRP").Value)
            oExcel.Application.Cells(i, "R").Value = IIf(rstPrintOrderStatusRegister.Fields("Processing").Value = "O", "Old", IIf(rstPrintOrderStatusRegister.Fields("Processing").Value = "N", "New", "Revised"))
...
Private Function GetBreakupRecievedQty(ByVal xOrderNo As String, ByVal xRefCode As String) As String
 On Error GoTo ErrHandler
    ...
    'rstBreakup.Open "Select Date,Value1 From Tran3 where (VchType=2 OR VchType=4) AND RecType =4 And LTRIM(No) = '" & xOrderNo & "' Order By Date", CxnBusy, adOpenKeyset, adLockReadOnly
    
    'Dim p As String
    'p = "SELECT Date,Value1 FROM Tran3 WHERE Tran3.RefCode =" & xRefCode & " And Tran3.RecType=4 and Tran3.VchType =4 And Tran3.Date <= '" & GetDate(MhDateInput2.Text) & "' ORDER BY Tran3.Type"
    'rstBreakup.Open "SELECT Date,Value1 FROM Tran3 WHERE Tran3.RefCode =" & xRefCode & " And Tran3.RecType=4 and Tran3.VchType =4 And Tran3.Date <= '" & GetDate(MhDateInput2.Text) & "' ORDER BY Tran3.Type", CxnBusy, adOpenKeyset, adLockReadOnly
...
Private Function BreakupExist(ByVal xOrderNo As String, ByVal oQty As Double) As Boolean
  On Error GoTo ErrHandler
    ...
    'rstExist.Open "SELECT * FROM (SELECT RefCode,No,Date,MasterCode1,SUM(ABS(Value1)) As OrderedQuantity,(SELECT ISNULL(SUM(ABS(Value1)),0) FROM Tran3 WHERE (VchType=2 OR VchType=4) AND RecType=4 AND RefCode=T.RefCode) As ReceivedQuantity FROM Tran3 T WHERE VchType=13 GROUP BY RefCode,No,Date,MasterCode1) As Tbl WHERE LTRIM(No)='" & xOrderNo & "' AND ReceivedQuantity>0", CxnBusy, adOpenKeyset, adLockReadOnly
